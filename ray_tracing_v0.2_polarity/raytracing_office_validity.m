...
n_window = [0,1,0,-6.8];
n_face = [n_floor_face;n_ceiling_face;n_leftwall_face;n_rightwall_face;n_frontwall_face;n_behindwall_face;n_obwall_face;n_window];
...
plot3(p_floor(:,1),p_floor(:,2),p_floor(:,3),'black-');
hold on;
...
plot3(p_ceiling(:,1),p_ceiling(:,2),p_ceiling(:,3),'-');
hold on;
...
plot3(p_leftwall(:,1),p_leftwall(:,2),p_leftwall(:,3),'-');
hold on;
...
plot3(p_rightwall(:,1),p_rightwall(:,2),p_rightwall(:,3),'-');
hold on;
...
plot3(p_frontwall(:,1),p_frontwall(:,2),p_frontwall(:,3),'-');
hold on;
...
plot3(p_behindwall(:,1),p_behindwall(:,2),p_behindwall(:,3),'-');
hold on;
 ...
 plot3(p_obwall(:,1),p_obwall(:,2),p_obwall(:,3),'-');
 fill3(p_obwall(:,1),p_obwall(:,2),p_obwall(:,3),'g')
 hold on;
%  add Window 
...
plot3(p_window(:,1),p_window(:,2),p_window(:,3),'--');
hold on;
...
p_allface = [p_floor, p_ceiling, p_leftwall, p_rightwall,p_frontwall,p_behindwall,p_obwall,p_window];
...
e_obwall=permitivity_obwall-1j*60*conductivity_obwall*wavelength;  %远轴截面
e_window = permitivity_window-1j*60*conductivity_window*wavelength;  %远轴截面
...
plot3([tx,tx],[ty,ty],[0,tz],'k')
plot3([tx,tx-0.2],[ty,ty],[tz-0.2,tz],'k')
plot3([tx,tx+0.2],[ty,ty],[tz-0.2,tz],'k')
plot3([rx,rx],[ry,ry],[0,rz],'k')
plot3([rx,rx-0.2],[ry,ry],[rz-0.2,rz],'k')
plot3([rx,rx+0.2],[ry,ry],[rz-0.2,rz],'k')
...
for n = 1:num_theta
    for m = 1:num_phi(n)
        ...
        if intersect_index == 1
            parameterR{n,m} = receive_LOS(v1,v2,v3,t1,t2,t3,rx,ry,rz,delta_theta,wavelength,GainT);  %如何判断中间是否有遮挡物
 ...
 %       rcoe = zeros(1,reflect_time);
        ...
        for i = 1:reflect_time
            ...
            [newray,rcoe(1,i)] = reflected(rp{1,i},v1,v2,v3,n_face,e_face);
            ...
            if isempty(parameterR{n,m}) == 1;
                intersect_index = intersection_judge(v1,v2,v3,t1,t2,t3,rx,ry,rz,n_face,p_allface);
                if intersect_index == 1
                    parameterR{n,m} = receive(t1,t2,t3,rx,ry,rz,newray,delta_theta,wavelength,pathlength,rcoe,GainT);
                    if isempty(parameterR{n,m}) == 0
...
E_sum_timedelay=sum(E_received.*time_delay);%
E_sum_timedelay_pow=sum(E_received.*time_delay.^2);%
tave=E_sum_timedelay/E_sum;%
et=E_sum_timedelay_pow/E_sum ;%;
...
k=(abs(E_los))^2/(abs(power_received_sum));%??????
hmatrix=sum(E_loss);%传输矩阵的元素????????
K_dB=10*log10(k);%K因子
pathloss_sum = power_tx./power_received_sum;%路径损耗
