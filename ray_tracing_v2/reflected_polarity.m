function [newray,rcoe_ver,rcoe_para,parallel_ref,vertical_ref] = reflected_polarity(rp,vx,vy,vz,n_face,e_face)
...
if norm(vertical_in)~=0
    ...
    rcoe_para= parallel_in*R_parallel;%平行极化反射系数
    %rcoe_ver = vertical_in.*vertical_ref.*R_vertical;%垂直极化反射系数
    rcoe_ver = vertical_in*R_vertical;%垂直极化反射系数
    %rcoe = rcoe_para+rcoe_ver;
else
    rcoe_para = R_parallel;
    rcoe_ver = 0;
    parallel_ref = [0,0,-1];
    parallel_in = [0,0,1];
    vertical_ref = [0,0,0];
