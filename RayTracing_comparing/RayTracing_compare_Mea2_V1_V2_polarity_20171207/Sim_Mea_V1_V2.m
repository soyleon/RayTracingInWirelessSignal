...
for  ant_temp = 1:antenna_num
    ...
    %%
    load(['E:\fuqiuhao\Raytraing_Platform\raytracing_matlab_simulation\ray_tracing_office_fuqiuhao\rayTracing_data\rayTracing_v2_indoor_tx1_rx1\20171207_polarity\indoor_Rv0.2_antenna',num2str(ant_temp),'.mat']);
    ...
    for num_ray_i = 1:num_ray(1,2)
        if time_delay(1,num_ray_i) == 0
            ...
            ZenithOA(ant_temp,num_ray_i) = acosd(dot(AOA_vec(1:3,num_ray_i),[0,0,1]));
            if AOA_vec(2,num_ray_i)>=0
                AzimuthOA(ant_temp,num_ray_i) = acosd(dot(AOA_vec(1:3,num_ray_i),[1,0,0]));
            else
                AzimuthOA(ant_temp,num_ray_i) = 360-acosd(dot(AOA_vec(1:3,num_ray_i),[1,0,0]));
            end
            ZenithOD(ant_temp,num_ray_i) = acosd(dot(AOD_vec(1:3,num_ray_i),[0,0,1]));
            if AOD_vec(2,num_ray_i)>=0
                AzimuthOD(ant_temp,num_ray_i) = acosd(dot(AOD_vec(1:3,num_ray_i),[1,0,0]));
            else
                AzimuthOD(ant_temp,num_ray_i) = 360-acosd(dot(AOD_vec(1:3,num_ray_i),[1,0,0]));
            end
        end
    end
...
figure(1);
plot(Delay,pdp_mea_norm_tar(30,:),'-g',time_delay_v1_norm(34,:),power_received_ray_dB_v1_norm(34,:),'-b',time_delay_v2_norm(34,:),power_received_ray_dB_v2_norm(34,:),'--r','linewidth',2);
title('Power Delay Profile');
xlabel('Delay (ns)');
ylabel('PDP(Level to the Peak, dB)');
legend('Measured','Ray tracing simple','Ray tracing considering polarity')
...
figure(2)
plot(1:antenna_num,-power_mea_dB,'-g',1:antenna_num,-pathloss_v1_dB,'-b',1:antenna_num,-pathloss_v2_dB,'--r','linewidth',2);
%title('Path Loss')
xlabel('Antenna ID');
ylabel('Path Loss(dB)');
legend('Measured','Ray tracing simple','Ray tracing considering polarity')
